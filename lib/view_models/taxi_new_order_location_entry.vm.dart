...
class NewTaxiOrderLocationEntryViewModel extends MyBaseViewModel {
  ...
  double customViewHeight2 = AppUISizes.taxiNewOrderIdleHeight2;
  ...
  destinationChange(bool value) {
    isDestinationSelected = value;
  ...
  onAddressSelected1(Address address) async {
    AlertService.showLoading();
    ...
    try {
      ...
      taxiViewModel.deliveryAddress = mDeliveryAddress;
      taxiViewModel.checkout?.deliveryAddress = mDeliveryAddress;
      taxiViewModel.pickupLocation=mDeliveryAddress;
      taxiViewModel.pickupLocation!.name=address.featureName;
       destinationChange(true);

      print("ETETETETE ${taxiViewModel.pickupLocation!.name}");
  ...
  ///
  moveToNextStep(String type) async {
    print("TEST PICK UP LOCATIOn ${taxiViewModel.pickupLocation }");
    print("TEST DROP OFF ${taxiViewModel.dropoffLocation }");
    if (taxiViewModel.dropoffLocation == null) {
      ...
      taxiViewModel.checkLocationAvailabilityForStep2(type);
    }
  }
}
